...
/* F.S. L terdefinisi dengan PAwal(L) = P1 dan PAkhir(L) = P2 */
...
/* Contoh:
   ...
   2 3
   3 3 
   --> terbentuk garis L dengan PAwal(L) = <2,3> dan PAkhir(L) = <3,3> */
{
	POINT P1, P2;
	BacaPOINT(&P1);
	BacaPOINT(&P2);
...
{
	return (sqrt((Ordinat(PAwal(L)) - Ordinat(PAkhir(L))) * (Ordinat(PAwal(L)) - Ordinat(PAkhir(L))) + (Absis(PAwal(L)) - Absis(PAkhir(L))) * (Absis(PAwal(L)) - Absis(PAkhir(L)))));
}
